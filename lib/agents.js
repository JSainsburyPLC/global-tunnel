...
function mixinProxying(agent, proxyOpts) {
  ...
  agent.createConnection = function(options) {
    ...
    if (this.proxy.no_proxy) {
      this.proxy.no_proxy.forEach(function(domain) {
        if (options['host'].indexOf(domain) !== -1) {
          noproxy = true;
        }
      });
    }
    ...
    if (noproxy) {
      return orig.createConnection.call(this,
                                        options['port'],  options['host'], options);
    }
  ...
  agent.addRequest = function(req, host, port, localAddress) {
    ...
    return orig.addRequest.call(this, req, host['host'], host['port'], localAddress);
  };
}
